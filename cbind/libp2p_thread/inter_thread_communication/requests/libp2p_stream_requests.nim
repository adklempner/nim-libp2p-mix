...
import pkg/libp2p_mix/curve25519 as mix_curve25519
...
proc processMixDial*(
  ...
  let stream = mixProto.toConnection(
...
proc processMixSetNodeInfo*(
  ...
  if libp2p[].mix.isNone:
    var mixProto = MixProtocol.new(
      libp2p[].mixNodeInfo.get(), libp2p[].switch, spamProtection = makeSpamProtection()
    )
    try:
